...
BUILD_RESULT=$(curl -u "${USERNAME}:${PASSWORD}" -X POST "${IDA_HOST}/rest/v2/pipelines/builds" -k -s --data-urlencode "pipelineName=${PIPELINE_NAME}" -d "{}"  -H "accept: application/json;charset=UTF-8" -H "Content-Type: application/json")
echo $BUILD_RESULT
...
PIPELINE_ID="$(cut -d',' -f2 <<<"$BUILD_STATUS")"
PIPELINE_ID="$(cut -d':' -f2 <<<"$PIPELINE_ID")"
